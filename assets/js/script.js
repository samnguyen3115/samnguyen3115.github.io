$(document).ready(function () {
  ...
  $("#contact-form").submit(function (event) {
    ...
    emailjs
      .sendForm("service_xhqg7zs", "template_bvvou4r", "#contact-form")
      .then(
        function (response) {
    ...
    container.addEventListener("click", function () {
      ...
      if (data) {
        ...
        if (data.reports.length > 0) {
          modalContent += '<div class="achievement-section">';
          modalContent += "<h3>Reports</h3>";
          data.reports.forEach((report) => {
            modalContent += `
                            <div class="achievement-item report-button" onclick="window.open('${report.link}', '_blank')" style="cursor: pointer;padding:0px;padding-top:5px">
                                <h4 style = "text-align: center;padding:0px; ;font-size:30px">View Report</h4>
                            </div>
                        `;
          });
          modalContent += "</div>";
